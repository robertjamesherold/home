...
export function SliderCardComponent({ slide, renderContent }: { slide: SliderCard; renderContent?: (s: SliderCard) => React.ReactNode }) {
  const Default = (
    <div className="flex max-w-xl flex-col gap-5 sm:gap-6">
      ...
      <h2 className="text-3xl font-semibold text-slate-900 sm:text-4xl md:text-5xl md:leading-[1.1]">{slide.title}</h2>
      <p className="text-base leading-relaxed text-slate-600 sm:text-lg">{slide.text}</p>
      {slide.ctaLabel && slide.ctaHref && (
        <a href={slide.ctaHref} className="inline-flex w-fit items-center gap-3 text-xs font-semibold uppercase tracking-[0.35em] text-slate-900 hover:text-slate-600">
