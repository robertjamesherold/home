...
const Navigation: React.FC<{ onToggleCart:()=>void, cartCount: number, isMobile: boolean }> = ({ onToggleCart, cartCount, isMobile }) => {
    const navItems = NavItemsData
    const horizontalPadding = useHorizontalPadding()

    return (

    <header className={`flex flex-row w-full justify-between py-2 bg-gray-900 ${horizontalPadding}`}>     
