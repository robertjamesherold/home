...
const SearchBar: React.FC<Props> = ({ value, onChange, className = '' }) => (
  <div
    className={`flex w-full items-stretch overflow-hidden rounded-full border border-purple-200/60 bg-white/90 shadow-lg shadow-purple-100/50 backdrop-blur focus-within:border-purple-400 focus-within:ring-4 focus-within:ring-purple-200/70 ${className}`}
  >
    <button
      type="button"
    ...
    <div className="relative flex-1">
      <input
        ...
        className="h-full w-full border-none bg-transparent px-4 py-3 pr-12 text-sm text-gray-900 outline-none placeholder:text-gray-400"
      />
      {value && (
        <button
          onClick={() => onChange('')}
