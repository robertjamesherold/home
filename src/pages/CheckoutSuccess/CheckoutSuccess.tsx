...
const CheckoutSuccess: React.FC = () =>
{
  ...
  const orderNumber = state.orderNumber ?? `LX-${ Math.floor( Math.random() * 9_999 ) }`
    ...
    <section className="container mx-auto px-4 py-10">
      <div className="flex flex-col items-center text-center">
        ...
        <p className="mt-2 max-w-xl text-gray-500">
          Ihre Bestellung ist bestätigt. Wir benachrichtigen Sie, sobald das Paket unser minimalistisches Lager
          verlässt.
        </p>
      </div>
      ...
      <Card className="mx-auto mt-8 max-w-3xl border border-gray-100 bg-white shadow-sm">
        <CardHeader>
          <CardTitle className="flex flex-wrap items-center justify-between text-base text-gray-900 sm:text-lg">
            ...
            <span className="text-sm font-normal text-gray-500">Minimal Studio Fulfillment</span>
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4 text-sm text-gray-600">
          <div className="space-y-3">
            { items.length === 0 && <p className="text-gray-500">Keine Positionen verfügbar.</p> }
            { items.map( ( item ) => (
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between">
              <span>Versand</span>
              <span className="text-gray-900">{ shippingCost === 0 ? 'Kostenlos' : `${ shippingCost.toFixed( 2 ) }€` }</span>
            </div>
