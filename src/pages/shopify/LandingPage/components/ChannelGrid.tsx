...
const ChannelGrid: React.FC = () => {
  ...
  return (
    <section className="space-y-4" data-section="channels">
      ...
      <Title level={4} weight="bold" className="text-white" text="Prime Video Channels" />
      <Title className="text-white/60" text="Fügen Sie Ihre Lieblingssender hinzu – jederzeit kündbar, direkt über Prime Video." />
    </header>
      ...
      {channels.map((channel) => (
        ...
        >
          ...
          <div className="relative z-10 flex h-full flex-col justify-end space-y-3 p-6">
            ...
            <Title className="uppercase tracking-wide text-[#00a8e1]" text={channel.price}/>
          </div>
        </article>
      ))}
    </div>
