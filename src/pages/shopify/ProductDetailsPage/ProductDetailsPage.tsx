...
import { ShoppingCart, Star, Truck, Shield, Check, Share2 } from 'lucide-react'
...
import FavoritenButton from './ui/Buttons/FavoritenButton'
import MengeButton from './ui/Buttons/MengeButton'
import Button from '@/ui/Buttons/Button'
import { Column, Grid } from '@/layout'
import GrößeButton from './ui/Buttons/GrößeButton'
import ColorButton from './ui/Buttons/ColorButton'
import DiscountBadge from './ui/Buttons/DiscountBadge'
import BigImage from './components/BigImage'


const createGalleryImages = ( src: string ): string[] =>
{
...
const ProductDetailPage: React.FC = () =>
{
  ...
  const availableSizes = useMemo( () =>
  {
    ...
    return CATEGORY_SIZES[ product.category ] ?? DEFAULT_SIZES
  }, [ product ] )
  ...
  const galleryImages = createGalleryImages( product.image )
  ...
  const discount = Math.max( 0, Math.round( ( ( originalPriceValue - priceValue ) / originalPriceValue ) * 100 ) )
  ...
  const specifications = CATEGORY_SPECIFICATIONS[ product.category ] ?? DEFAULT_SPECIFICATIONS
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          <div className="space-y-4">
            <div className="relative bg-white rounded-2xl shadow-xl overflow-hidden aspect-square">
              <BigImage images={ galleryImages } title={ product.title } />
              <DiscountBadge product={ product } originalPrice={ originalPrice } />
              <FavoritenButton />
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-baseline space-x-3">
              <span className="text-4xl font-bold text-purple-600">{ priceDisplay }</span>
              { discount > 0 && (
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-green-600 font-semibold">Sofort verfügbar</span>
            </div>

            <p className="text-gray-600 leading-relaxed">{ product.description }</p>

            <ColorButton availableColors={ availableColors } />
            <GrößeButton availableSizes={ availableSizes } />
            <MengeButton />
            <Column className="gap-4 mt-4">
              <Button variant="primary" label="Jetzt kaufen"><ShoppingCart /></Button>
              <Button variant="secondary" label="In den Warenkorb"></Button>
              <Button variant="outline" label="Produkt teilen"><Share2 /></Button>
            </Column>
            ...
            <div className="grid grid-cols-3 gap-4 pt-6 border-t">
              <div className="flex flex-col items-center text-center space-y-2">
                ...
                <span className="text-sm font-semibold">Versand am selben Tag</span>
              </div>
              <div className="flex flex-col items-center text-center space-y-2">
                <div className="bg-purple-100 p-3 rounded-full">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-gray-700 leading-relaxed">
                    Kostenloser Premium-Versand innerhalb Deutschlands ab 50 €. Bestellungseingänge bis 14 Uhr werden noch am selben Tag bearbeitet.
                  </p>
                </div>
                <div>
                  <h3 className="text-xl font-bold mb-3">Retouren & Service</h3>
                  <p className="text-gray-700 leading-relaxed">
                    30 Tage Rückgaberecht. Ungetragene Ware kann kostenfrei retourniert werden. Rückerstattungen erfolgen innerhalb von 5–7 Werktagen.
                  </p>
                </div>
              </div>
            ) }
